...
class CustomersController {
    ...
    constructor(router) {
        ...
        router.post('/location', this.addLocation.bind(this));
    }
    ...
    addLocation(req, res) {
        ...
        customersRepo.addLocation(req.query, (err, customer) => {
            if (err) {
                console.log('*** addLocation error: ' + util.inspect(err));
                res.json({ status: false });
            } else {
                console.log('*** addLocation ok');
                res.json(customer);
            }
        });
    }
